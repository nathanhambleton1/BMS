...
if npulse > 0
    ...
    chk(end+1,:) = {'pulse current stayed under the discharge trip', ...
        max(Imeas) < p.Bms.I_dch_trip, ...
        sprintf('peak %.2f A against a %.0f A trip (%.0f%% margin)', ...
        max(Imeas), p.Bms.I_dch_trip, 100*(p.Bms.I_dch_trip/max(Imeas)-1))};
end
...
if any(chgMask)
    ...
    chk(end+1,:) = {'charge current sits at the CC setpoint', ...
        ~isempty(settled) && abs(median(settled) - p.Charger.I_cc_A) < 0.05, ...
        sprintf('median %.3f A against I_cc_A = %.3f A', median(settled), p.Charger.I_cc_A)};
