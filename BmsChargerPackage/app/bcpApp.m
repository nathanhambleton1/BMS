    ...
    methods (Access = private)
        ...
        function buildBmsTab(obj)
            ...
            obj.numField(f, 'Current sign', 'Bms.I_sign', '+1/-1', ...
                ['-1 if your battery model reports discharge as POSITIVE current, ' ...
                 'which is what Simscape Battery blocks do. Get this wrong and the ' ...
                 'BMS over-current-trips on a charge and never trips on a ' ...
                 'discharge. Check it once against a known discharge.']);
            ...
            obj.note(R, 'Current protection, pack', true);
            f = obj.fieldGrid(R, 3);
            obj.numField(f, 'Charge trip', 'Bms.I_chg_trip', 'A', ...
                ['A FAULT threshold, which must sit above the current the charger ' ...
                 'is entitled to command. Over-current is the one fault that ' ...
                 'opens the pack in both directions.']);
            obj.numField(f, 'Discharge trip', 'Bms.I_dch_trip', 'A', ...
                'Magnitude. Compare it against the peak current your load implies.');
            obj.numField(f, 'Current confirm time', 'Bms.t_i_trip', 's', ...
            ...
            obj.numField(f, 'Charge current margin', 'Bms.I_chg_margin', '0..1', ...
                ['The charger is never permitted more than this fraction of the ' ...
                 'charge trip, so it cannot command a current that trips its own ' ...
                 'protection.']);
        ...
        function buildChargerTab(obj)
            ...
            f = obj.fieldGrid(g, 4);
            obj.numField(f, 'CC setpoint', 'Charger.I_cc_A', 'A', ...
                'The constant-current phase. Auto-fill uses the datasheet value.');
            ...
            obj.numField(f, 'Supply power ceiling', 'Charger.P_chg_max_W', 'W', ...
                ['Applied after the control law, because a real supply simply ' ...
                 'cannot exceed it however the loop reasons about it.']);
            ...
            obj.note(g, ['Auto-fill never uses a datasheet MAXIMUM charge current. ' ...
                'Those are qualified by a cell-temperature cutoff and this package ' ...
                'has no thermal model to enforce one, so the default is always the ' ...
                'standard charge. Ask for a higher C-rate explicitly if you mean it.']);
        end
        ...
        function onChargeTime(obj)
            t = obj.Proj.Charger.estimatedChargeTime_h(obj.Proj.Pack) * 3600;
            obj.log('CC-phase charge from 20%% to 95%%: about %.0f s (%.2f h) at %.2f A into %.1f Ah.', ...
                t, t/3600, obj.Proj.Charger.I_cc_A, obj.Proj.Pack.Q_Ah);
            ...
            if meanW > 0
                ...
                if meanA >= obj.Proj.Charger.I_cc_A
        ...
        function refreshDerived(obj)
            ...
            obj.setText('PackR', sprintf('%.1f mOhm   (datasheet estimate, not measured)', ...
                p.R_dc_Ohm*1000));
            obj.setText('PackIch', sprintf('%.1f A standard (%.2fC)     %.1f A maximum', ...
                p.I_chg_std_A, p.Cell.C_rate_chg(), p.I_chg_max_A));
            obj.setText('PackIdch', sprintf('%.0f A continuous     %.0f W at nominal', ...
                p.I_dch_A, p.P_dch_W));
