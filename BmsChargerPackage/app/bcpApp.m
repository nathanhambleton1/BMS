...
%   real reason appears in the log at the bottom.
    ...
    methods (Access = private)
        ...
        function buildBmsTab(obj)
            ...
            % --- left column -----------------------------------------------
            L = uigridlayout(g, [11 1]);
            L.RowHeight = repmat({'fit'},1,11);
            L.Scrollable = 'on';
            ...
            obj.chkField(f, 'Clamp reported SOC to 0..1', 'Bms.SOC_clamp', ...
                ['LEAVE THIS ON. A Simscape table_battery integrates coulombs ' ...
                 'with no floor, so a pack discharged past its rated capacity ' ...
                 'reports a NEGATIVE state of charge and goes on doing so -- the ' ...
                 'component clamps its OCV and resistance tables at the ends but ' ...
                 'not the integrator. Every SOC comparison in this package is ' ...
                 'written against 0..1, so the clamp is what keeps them meaning ' ...
                 'something. The unclamped value is still published on diag(17), ' ...
                 'so the excursion stays visible.']);
            ...
            obj.note(L, ['Fault handling. The last four exist because a fault the ' ...
                'protection layer CURES by acting on it will re-trip the moment ' ...
                'it recovers -- an under-voltage under a load is exactly that, ' ...
                'since the sag IS the load. Left alone that cycle costs energy ' ...
                'every lap and returns none, so the dips grow.'], true);
            f = obj.fieldGrid(L, 6);
            ...
            obj.numField(f, 'UV needs charge back in', 'Bms.Q_uv_reset_Ah', 'Ah', ...
                ['Amp-hours of charge the pack must take on before an ' ...
                 'under-voltage latch will clear. A cell that returns to 3.0 V ' ...
                 'when the load is removed has stopped losing energy, not gained ' ...
                 'any, and it is still at its end of discharge -- so a rest is ' ...
                 'not a recovery. ON A LOAD-ONLY RUN THIS MAKES THE UV LATCH ' ...
                 'PERMANENT, which is the correct end of a discharge test. Set it ' ...
                 'to 0 for the old rest-is-enough behaviour.']);
            obj.numField(f, 'Retry backoff factor', 'Bms.Retry_Backoff_x', 'x', ...
                ['Each automatic recovery multiplies the required dwell by this. ' ...
                 'A fault retry counter with exponential backoff, as on anything ' ...
                 'that reconnects itself to a load: whatever oscillation survives ' ...
                 'everything else gets slower each lap instead of faster. 1 ' ...
                 'disables the escalation.']);
            obj.numField(f, 'Retry window', 'Bms.t_retry_window_s', 's', ...
                ['Run this long with nothing latched and the retry counter ' ...
                 'resets. It measures time spent running CLEAN, which is the only ' ...
                 'reading that means what the counter is for.']);
            obj.numField(f, 'Retries before lockout', 'Bms.N_retry_max', 'count', ...
                ['Automatic recoveries allowed before auto-recovery is withdrawn ' ...
                 'entirely -- past this, only a reset edge clears anything and ' ...
                 'state reports 5 (LOCKOUT) rather than 4 (FAULT). A pack that ' ...
                 'faulted, recovered and faulted again three times in half a ' ...
                 'minute has one fault that recovery does not fix, not three. ' ...
                 'Type Inf to leave only the backoff; 0 makes the first fault ' ...
                 'terminal.']);

            obj.note(L, ['Discharge limiting. This is the main answer to the ' ...
                'cycle described above: derate the load CONTINUOUSLY as the pack ' ...
                'approaches its limits, the way a production BMS publishes a ' ...
                'discharge limit the inverter must obey, so the trip stays a ' ...
                'backstop instead of doing the regulating.'], true);
            f = obj.fieldGrid(L, 9);
            obj.chkField(f, 'Load limiter', 'Bms.UseLoadLimiter', ...
                ['On by default. Off leaves the trip as the only discharge limit, ' ...
                 'which on a constant-power load is a bang-bang loop -- cutting ' ...
                 'the load removes the sag that tripped it, the fault clears, the ' ...
                 'load returns and sags deeper. Turn it off only to reproduce a ' ...
                 'result from before the limiter existed.']);
            obj.numField(f, 'Foldback margin above UV', 'Bms.V_fold_margin_V', 'V', ...
                ['The load is fully derated this far ABOVE the under-voltage ' ...
                 'trip. The limiter regulates the lowest cell into a band just ' ...
                 'above the trip, so this margin is what the trip keeps for ' ...
                 'itself. Too small and the limiter has no room to act first.']);
            obj.numField(f, 'Foldback band width', 'Bms.V_fold_band_V', 'V', ...
                ['Full load is allowed at (UV trip + margin + this) and above. ' ...
                 'The limiter holds still anywhere inside the band, which is what ' ...
                 'stops it hunting. Keep your design pulse''s worst-case sag ' ...
                 'ABOVE the top of the band, or the test is being limited rather ' ...
                 'than run.']);
            obj.numField(f, 'Current foldback start', 'Bms.I_fold_frac', 'x fast trip', ...
                ['Current foldback starts at this fraction of the FAST discharge ' ...
                 'trip and reaches full derate at it. Keyed to the fast tier on ' ...
                 'purpose: the sustained tier exists to be exceeded briefly by a ' ...
                 'pulse the pack is rated for, so folding back against it would ' ...
                 'derate the designed test.']);
            obj.numField(f, 'Closing rate (cap)', 'Bms.Fold_Fall_per_s', '1/s', ...
                ['Fastest the limit closes. It is a CAP -- the actual rate is ' ...
                 'proportional to how far outside the band the pack is, which is ' ...
                 'what makes the loop stable near the pack''s maximum power ' ...
                 'point, where dV/dP is unbounded.']);
            obj.numField(f, 'Reopening rate', 'Bms.Fold_Rise_per_s', '1/s', ...
                ['Rate the limit reopens. Much slower than closing, and the ' ...
                 'asymmetry is the point: cells sag in milliseconds and recover ' ...
                 'over seconds. A limiter that reopened as fast as it closed ' ...
                 'would be the trip again with extra steps.']);
            obj.numField(f, 'Minimum load fraction', 'Bms.Fold_Floor', '0..1', ...
                ['Smallest fraction of the demand the limiter will allow. 0 lets ' ...
                 'it take the load to zero, which is still better than a trip: no ' ...
                 'latch, no contactor event, and it reopens on its own.']);
            obj.numField(f, 'Reopen needs current above', 'Bms.Fold_Learn_frac', ...
                'x fold start', ...
                ['The limit only reopens while current is actually flowing. At ' ...
                 'rest the lowest cell relaxes to OCV, far above the band, and ' ...
                 'reopening on that reading would arrive at the next pulse having ' ...
                 'forgotten what the last one proved -- an open-circuit voltage ' ...
                 'is not evidence the pack can deliver power. Load or charge both ' ...
                 'count as evidence; rest does not.']);
            obj.numField(f, 'Soft-start ramp', 'Bms.t_softstart_s', 's', ...
                ['Re-engage the load over this long after a discharge inhibit ' ...
                 'clears, rather than in one sample. An inverter soft-starts for ' ...
                 'the same reason, and in simulation a step re-engagement is the ' ...
                 'surest way back under the threshold before the limiter has had ' ...
                 'a sample to react. 0 disables it.']);
        ...
        function buildInstallTab(obj)
            ...
            obj.note(g, ['The generated blocks paste this package''s alg/ code ' ...
                'in as local functions, so a model containing them compiles on ' ...
                'its own -- send it to a colleague, or copy just the two blocks ' ...
                'into a new project, with nothing else from this package needed.']);
        end
