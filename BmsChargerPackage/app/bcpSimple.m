...
%     trips computed from it three lines later.
    ...
    methods (Access = private)
        ...
        function refresh(obj)
            ...
            % The discharge limiter belongs in this note rather than only in the
            % full window, because it changes what the pack DOES near empty --
            % the load is derated instead of the trip firing -- and someone
            % reading a run needs to know that before they read it as a fault.
            if p.Bms.UseLoadLimiter
                limTxt = sprintf( ...
                    [' Discharge derated continuously below %.3f V/cell and fully ' ...
                     'at %.3f V/cell, so the under-voltage trip is a backstop ' ...
                     'rather than the operating limit.'], ...
                    p.Bms.V_fold_start(), p.Bms.V_fold_end());
            else
                limTxt = [' Discharge limiting is OFF, so the trip IS the ' ...
                          'operating limit -- on a constant-power load near ' ...
                          'empty that cycles.'];
            end
            obj.Ctl.PackNote.Text = sprintf( ...
                ['Protection derived from the datasheet: cell trips %.2f V high / ' ...
                 ...
                 'discharge %.0f A over %.0f s or %.0f A over %.2f s.%s'], ...
        ...
        %   here, on a machine with an empty base workspace.
            ...
            try
                ...
                obj.log(['Both blocks are self-contained -- copy them into any ' ...
                         'project and they compile with nothing else from this ' ...
                         'package. Wire your battery to the BMS inports and the ' ...
