...
%     widths (bcp.Project.verifyWiring) rather than guessing from a file.
    ...
    methods
        ...
        %
        %     from the TABLES   capacity and resistance. Those are model
        %                       parameters, and they are what your Simscape pack
        %                       will actually behave like.
        %     from the DATASHEET  cutoff voltages, all four current ratings and
        %                       the temperature ranges. None of those exist in
        %                       the tables, because they are not model
        %                       parameters -- they are facts about how hard you
        %                       may drive the cell, and they are what every
        %                       protection threshold is derived from.
        %
        %   THE DATASHEET ENTRY IS LOOKED UP BY PART NUMBER
        %     The generated .ssc carries the part number in its header comment,
        %     so a pack built from a P45B gets the P45B's real limits: 13.5 A
        %     maximum charge, 45 A continuous discharge, 0-60 degC charge.
        %     Earlier versions hard-coded a single set of defaults here, and the
        %     effect was that a tables-backed cell silently lost its datasheet
        %     maximum charge current -- so the UI capped the charge rate at the
        %     4.35 A standard charge with no way to see why.
        %
        %     Pass 'Datasheet', someCellLibraryEntry to override the lookup, or
        %     any individual field by name.
        %
        ...
        %   auto-fill can use; the harness plant gets the whole curve.
            ds = obj.matchDatasheet();
            opt = struct('R_at_SOC',0.5, 'Datasheet',ds);
            for k = 1:2:numel(varargin)
                if strcmp(varargin{k}, 'Datasheet')
                    ds = varargin{k+1};
                end
            end
            % Every datasheet-owned field defaults from the matched entry and
            % stays overridable one at a time.
            for f = {'V_nom','V_max','V_min','I_chg_std_A','I_chg_max_A', ...
                     'I_term_A','I_dch_cont_A','I_dch_pulse_A', ...
                     'T_chg_min_C','T_chg_max_C','T_dch_min_C','T_dch_max_C', ...
                     'Mass_kg','Chemistry'}
                opt.(f{1}) = ds.(f{1});
            end
            for k = 1:2:numel(varargin)
            ...
            c = bcp.CellLibrary( ...
                ...
                'I_term_A',      opt.I_term_A, ...
                'I_dch_cont_A',  opt.I_dch_cont_A, ...
                'I_dch_pulse_A', opt.I_dch_pulse_A, ...
                'T_chg_min_C',   opt.T_chg_min_C, 'T_chg_max_C', opt.T_chg_max_C, ...
                'T_dch_min_C',   opt.T_dch_min_C, 'T_dch_max_C', opt.T_dch_max_C, ...
                'Mass_kg',       opt.Mass_kg, ...
                'Source',        sprintf('%s (tables) + %s (limits)', ...
                                         obj.Source, ds.Source));
            c = c.validate();
        end

        function ds = matchDatasheet(obj)
        %MATCHDATASHEET  The library entry whose part number matches these tables.
        %
        %   Matched on the part number the .ssc header carries, e.g.
        %   "INR-21700-P45B" against "Molicel INR-21700-P45B". Falls back to the
        %   P45B when nothing matches, which is a guess -- so it says so in the
        %   returned entry's Source, and that string ends up on screen.
            ds = bcp.CellLibrary.P45B();
            if isempty(obj.Name), return; end
            names = bcp.CellLibrary.names();
            for k = 1:numel(names)
                if strcmp(names{k}, 'Custom'), continue; end
                if contains(names{k}, obj.Name, 'IgnoreCase', true)
                    ds = bcp.CellLibrary.byName(names{k});
                    return;
                end
            end
            ds.Source = sprintf( ...
                ['no library entry matches part number "%s" -- current and ', ...
                 'temperature limits are the P45B''s, CHECK THEM'], obj.Name);
        end
