...
%                         5 LOCKOUT (latched, auto-recovery withdrawn)
%           8 faults      latched bitmask: 1 OV, 2 UV, 4 OCc, 8 OCd, 16 OT, 32 UTc
%                         SIMULTANEOUS FAULTS ADD, so 5 = OCc+OV and 10 = OCd+UV.
%                         bcp.Signals.faultTable decodes every mask you will see.
%           9 diag        17-vector, see bcp.Signals.diagNames()
%
...
%     everything, and it is the simpler installation.
    ...
    methods (Access = private)
        ...
        %ANNOTATE  Leave the wiring contract on the canvas, next to the block.
            ...
            if c.UseLoadLimiter
                lim = sprintf('full load above %.3f V/cell, zero at %.3f V/cell', ...
                    c.V_fold_start(), c.V_fold_end());
            else
                lim = 'OFF -- the trip is the operating limit, and will chatter';
            end
            txt = sprintf([ ...
                ...
                'Out 9 diag is 17 wide and goes to a scope. Do not swap them.\\n' ...
                'DISCHARGE LIMITER: %s\\n' ...
                'Self-contained: alg/ is embedded in BMS_core. No bcp_setup needed to compile.'], ...
                c.Ts, pri, c.I_chg_max_A, ...
                c.I_chg_trip, c.t_i_cont_s, c.I_chg_peak_A, c.t_i_trip, lim);
            try
        ...
        %     it in the block is one edit that keeps protection consistent.
            ...
            lim  = bcp.Blocks.structLiteral(c.limiterParams(),    '        ');
...
'T_hot = max(Tmax(:));\n' ...
'\n' ...
'%%  1. Reduce the per-cell arrays to pack scalars. soc_raw is the UNCLAMPED\n' ...
'%%     [min; max] SOC the battery model reported -- kept for diag(17), because\n' ...
'%%     a Simscape table_battery integrates coulombs with no floor and will\n' ...
'%%     report a negative SOC forever once the pack is past empty.\n' ...
'[pack_meas, soc_raw] = bcp_pack_monitor(Vcell, SOCcell, Icell, MON);\n' ...
'\n' ...
...
'[contactor, chg_ok, dch_ok, state, faults, flags, pinfo] = bcp_protection( ...\n' ...
'    pack_meas(2), pack_meas(3), T_hot, pack_meas(7), reset > 0.5, PRO);\n' ...
'\n' ...
'%%  4. The load, limited. A discharge inhibit has to inhibit the discharge --\n' ...
'%%     the load is the only discharge path in this model -- but a BINARY\n' ...
'%%     inhibit on a constant-power load oscillates, because cutting the load\n' ...
'%%     removes the sag that tripped it. bcp_load_limiter puts a continuous\n' ...
'%%     limit in front of the trip, the way a production BMS publishes a\n' ...
'%%     discharge limit the inverter must obey, so protection stays a backstop.\n' ...
'%%     alg/bcp_load_limiter.m derives the oscillation and the fix in full.\n' ...
'%%\n' ...
'%%     The GATED, LIMITED value is what counts as "the load is active", and\n' ...
'%%     the order matters. Feeding the ungated demand to the arbiter would mean\n' ...
'%%     an under-voltage fault holds the load flag high, the arbiter sees a busy\n' ...
'%%     load and refuses to charge, and the pack sits at its floor forever --\n' ...
'%%     deadlocked by its own protection, with a charger idle beside it. The\n' ...
'%%     cure for under-voltage is a charge.\n' ...
'[P_load, dclFrac, limState] = bcp_load_limiter( ...\n' ...
'    P_demand, pack_meas(2), pack_meas(7), dch_ok, LIM);\n' ...
...
'\n' ...
'%%  Preallocated, so diag is 17 wide at compile time whatever code generation\n' ...
...
'diag(4) = dch_ok;\n' ...
'diag(5:10) = flags(1:6);\n' ...
'diag(11) = dclFrac;         %% 0..1, the fraction of demand allowed through\n' ...
'diag(12) = limState;        %% which limit is binding, bcp.Signals.limitState\n' ...
'diag(13) = pinfo(1);        %% automatic recoveries inside the retry window\n' ...
'diag(14) = pinfo(2);        %% 1 = auto-recovery withdrawn, reset only\n' ...
'diag(15) = pinfo(3);        %% Ah charged since the under-voltage latch\n' ...
'diag(16) = pinfo(4);        %% clear-band dwell recovery currently requires\n' ...
'diag(17) = soc_raw(1);      %% UNCLAMPED min SOC -- negative means past empty\n' ...
'end\n'], ...
                ...
                mon, pro, arb, lim, ldp, sgn);

            % The block used to CALL these five and rely on BmsChargerPackage/alg
            % being on the path. Pasting their source in as local functions makes
            % BMS_core self-contained: copy it (with Charger) into a new project,
            % delete this package, and it still compiles. See
            % bcp.Blocks.embedAlgFunctions for why the call sites above need no
            % change to find them.
            s = [s, bcp.Blocks.embedAlgFunctions({'bcp_tick', 'bcp_pack_monitor', ...
                'bcp_load_scheduler', 'bcp_protection', 'bcp_load_limiter', ...
                'bcp_arbiter'})];
        end
    end
end
