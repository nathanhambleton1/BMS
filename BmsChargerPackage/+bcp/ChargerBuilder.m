...
%
%     I_limit is also the CHARGE RATE, not just a safety ceiling. The generated
%     code carries this supply's own current and power ratings as I_SUPPLY_MAX_A
%     and P_SUPPLY_MAX_W, and commands the lower of those and I_limit. So there
%     is exactly one number to change to charge faster, it lives in the BMS
%     block, and nothing here needs to be kept in step with it.
%
%   PORT 1 IS 7 WIDE AND THE BMS's diag OUTPUT IS 10
%     pack_meas expects the BMS block's 7-element pack_meas output (output 5),
%     not its 10-element diag output (output 9). Wire the wrong one and the
%     error is a width mismatch reported against this block's input delay --
%     "expected [7], got [10]" -- a long way from the line that caused it.
%     Copying both blocks together with bcp.Project.stageBlocks (or the UI's
%     Copy models button) brings the five internal lines pre-drawn, which is the
%     reliable way to avoid the whole question.
%
    ...
    methods (Access = private)
        ...
        function annotate(obj)
            ...
            txt = sprintf([ ...
                ...
                'Supply rating %.2f A / %.0f W -> CV %.3f V/cell (%.1f V pack)\\n' ...
                'Terminates below %.2f A held %.1f s.\\n' ...
                'The CHARGE RATE is not set here -- it arrives on I_limit from the BMS.\\n' ...
                'Inputs come from the BMS block. It decides when and how fast;\\n' ...
                'this decides the CC/CV shape. P_chg_cmd and done go BACK to the BMS.'], ...
                c.Ts, c.I_cc_A, c.P_chg_max_W, c.V_cv_cell, c.V_cv_pack, ...
                c.I_taper_A, c.t_term_s);
            try
...
'\n' ...
'%%%% ===== WHAT THIS SUPPLY CAN SOURCE =================================\n' ...
'%%%%  These two are HARDWARE ratings, not the charge rate. The charge rate\n' ...
'%%%%  arrives on the I_limit input from the BMS block, and this block\n' ...
'%%%%  commands the lower of the two -- so to charge faster you change one\n' ...
'%%%%  number in the BMS (I_CHG_MAX_A), not anything here.\n' ...
'%%%%\n' ...
'%%%%  Change these only when you are modelling a different power supply.\n' ...
'%%%%  Setting either below the BMS limit makes this block the thing that\n' ...
'%%%%  limits the charge, which is legitimate but is worth knowing you did:\n' ...
'%%%%  bcp.Project.check() says so before you run.\n' ...
'I_SUPPLY_MAX_A = %s;\n' ...
'P_SUPPLY_MAX_W = %s;\n' ...
'%%%% ===================================================================\n' ...
'\n' ...
'P = %s;\n' ...
'P.I_cc_A = I_SUPPLY_MAX_A;\n' ...
'\n' ...
...
'if P_chg_cmd > P_SUPPLY_MAX_W\n' ...
'    P_chg_cmd = P_SUPPLY_MAX_W;\n' ...
'    if pack_meas(1) > 1\n' ...
